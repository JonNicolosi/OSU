...
url="http://vernal-parser-155402.appspot.com/"
...
# Adding two books
resp = r.put(url + '/books', json=b1)
assert resp.status_code == 201
knownBooks.append(j.loads(resp.text)['id'])

resp = r.put(url + '/books', json=b2)
assert resp.status_code == 201
knownBooks.append(j.loads(resp.text)['id'])
...
# Adding a customer
resp = r.put(url + '/customers', json=c1)
assert resp.status_code == 201
...
resp = r.get(url + "/books?checked_out=true")
assert resp.status_code == 200
assert len(j.loads(resp.text)) == 1
