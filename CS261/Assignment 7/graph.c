...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    int i;
	while (!dequeIsEmpty(searchStack))
	{
		Vertex* currentVert = dequeBack(searchStack);
		dequePopBack(searchStack);
		currentVert->isVisited = 1;
		if (currentVert == destination)
			return 1;

		for (i = 0; i < currentVert->numNeighbors; i++)
		{
			if (!(currentVert->neighbors[i]->isVisited))
				dequePushBack(searchStack, currentVert->neighbors[i]);
		}
	}
	//clearVisited(graph);
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	/* */
	Deque *searchQueue = dequeNew(searchQueue);


	dequePushBack(searchQueue, source);

	while (!dequeIsEmpty(searchQueue))
	{
		Vertex* currentVert = dequeFront(searchQueue);
		dequePopFront(searchQueue);
		currentVert->isVisited = 1;
		if (currentVert == destination)
			return 1;
		...
		{
			if (!(currentVert->neighbors[i]->isVisited))
				dequePushBack(searchQueue, currentVert->neighbors[i]);
		}
	}

	dequeDelete(searchQueue);
	free(searchQueue);
	return 0;
}
...
Graph* randomGraph(int numVertices, int numEdges)
{
    ...
    {
        Vertex* v1 = &graph->vertexSet[edges[k].i];
        Vertex* v2 = &graph->vertexSet[edges[k].j];
        createEdge(v1, v2);
    }
    free(edges);
...
Graph* loadGraph(const char* fileName)
{
    ...
    while (fgets(buffer, sizeof buffer, file) != NULL)
    {
        ...
        for (k = (int) strtol(begin, &end, 10);
            end != begin;
            k = (int) strtol(begin, &end, 10))
        {
            Vertex* neighbor = &graph->vertexSet[k];
